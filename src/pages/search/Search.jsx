    ...
    <div id="search-page"
      ...
      <div className="container max-w-[1280px] mx-auto">
        <div id="search-container" className={`w-full mx-auto pt-20 flex flex-col justify-center items-center 
          ...
          flex justify-between items-center text-[clamp(1.2rem,_2vw,_1.5rem)] cursor-pointer">
            <div>
              <h2>Search Results for "{query}"</h2>
            </div>
            <div id="icon-toggle-container" className="flex gap-2.5">
        ...
        m-auto bg-[var(--gray-10)] relative">
          {view === 'list' ? (
            <div className="aside-content h-auto text-black p-2.5">
              <ul>
                {result && result.articles.slice(0, 24).map((article, index) => (
                  <li
                    key={article.id}
            ...
            <div id="search-card-container"
              ...
              {
                result && result.articles.slice(0, 24).map(article => (
                  <Card
                    key={article.id}
...
function SearchMobileMenu({ mobileMenuIsOpen, setMobileMenuIsOpen }) {
  ...
  return (
    <div id="mobile-menu" className={`absolute top-5 right-[1%] sm:right-[4.3%] z-50 ${mobileMenuIsOpen ? 'isOpen' : ''}`}>
      <button
        type="button"
        id="mobile-nav-btn"
